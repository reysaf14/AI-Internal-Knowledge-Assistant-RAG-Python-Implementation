...
| `MODEL_BASE_URL` | Endpoint model generik | Required untuk bot runtime; test dapat menunjuk mock lokal | `local`, `test`, `vps` | `http://127.0.0.1:8080` | Human/Engineer berdasarkan runtime model yang dipilih | No | Model adapter; URL tervalidasi terhadap profile/allowlist target |
| `MODEL_NAME` | Identifier model generik | Required untuk bot runtime | `local`, `test`, `vps` | `local-default` | Human/Engineer melalui runtime profile | No | Model adapter; tidak boleh kosong atau placeholder saat runtime aktif |
| `MODEL_TIMEOUT_SECONDS` | Batas waktu pembangkitan jawaban | Required untuk bot runtime | `local`, `test`, `vps` | `3` | Architect/Engineer; harus menyisakan headroom dari `<5,0 detik` | No | Model adapter; positive numeric value, timeout menghasilkan fallback aman |
...
Semua path relatif terhadap root project. Precedence yang direncanakan: process environment atau injected runtime secret menang atas file profile; file profile hanya dipakai bila launcher memang secara eksplisit memuatnya; default aplikasi hanya berlaku untuk variabel optional. Nama sumber yang sama tidak boleh diam-diam mengambil nilai dari profile lain.

| Profile | Working directory / launcher | Sumber konfigurasi | Target dan boundary | Missing-value behavior |
|---|---|---|---|---|
| `local` | Root project; entry point Python ingest atau bot | Process environment; `.env` lokal hanya bila loader eksplisit mengaktifkannya | Folder `docs/`, SQLite lokal, model yang berjalan di endpoint lokal, dan bot yang diprovision Human | Required kosong/hilang/placeholder menghentikan startup sebelum network/write |
...
Engineer wajib membuat template root `.env.example` dengan nilai aman saja. Human mengisi credential melalui jalur runtime yang disetujui. `.env`, `.env.test`, backup, export credential, dan file key tidak boleh masuk Git atau log. Test profile tidak boleh membaca credential atau corpus production untuk menutup gap.

## Credential references

Tidak ada credential store n8n. Referensi credential yang direncanakan hanya `TELEGRAM_BOT_TOKEN` melalui runtime injection/credential store yang disetujui Human. Nilai token tidak diperlukan untuk desain, implementasi sintetik, atau report. Jika target VPS menyediakan secret store khusus, DevOps mencatat nama referensi aman dan source precedence tanpa menyalin nilainya.
